...
if (isset($_POST['place_order'])) {
    if ($user_id != "") {
        ...
        if (isset($_POST['method'])) {
            if ($_POST['method'] == "cash_on_delivery") {
                // Process cash on delivery order
                processCashOnDeliveryOrder($conn, $user_id, $name, $number, $email, $address, $address_type);
            } elseif ($_POST['method'] == "online_payment") {
...
// function processCashOnDeliveryOrder($conn, $user_id, $name, $number, $email, $address, $address_type) {
//     // Process cash on delivery order logic here
// }
...
// Prepare and execute SQL statement to insert order details into the database
    $insert_order = $conn->prepare("INSERT INTO orders (user_id, name, number, email, address, address_type, payment_method, total_amount, order_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $insert_order->execute([$user_id, $name, $number, $email, $address, $address_type, $payment_method, $total_amount, $order_status]);
...
">
    ...
    ">
        ...
        <h1 style="
            ...
            margin-top: 2%;
            text-align: center;
            font-size:50px;">Proceed for checkout </h1>
    </div>
</section>
